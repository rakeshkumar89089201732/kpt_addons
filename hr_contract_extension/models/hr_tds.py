...
class HrTDS(models.Model):
    ...
    def _onchange_tax_regime_slab(self):
        ...
        # Keep existing lines untouched and only append missing lines.
        # Rebuilding the whole one2many during onchange can leave unsaved
        # virtual rows in an invalid state and crash Odoo's diff snapshot.
        ...
        for scheme in available_schemes:
            if not scheme or not scheme.id:
                continue
            if existing_by_scheme_id.get(scheme.id):
                continue
