...
class MonthlyAttendanceSetWizard(models.TransientModel):
    ...
    total_present_days = fields.Integer(
        ...
        required=True,
        default=0,
        help='Number of days in the month to mark as present. Attendance can be created on any calendar day, including Sundays and public holidays.',
    )
    ...
    def _get_working_days_in_month(self):
        """
        Return a sorted list of all dates in the selected month.

        Sundays and public holidays are also eligible days; this method is
        used only to distribute the requested Total Present Days across the month.
        """
        self.ensure_one()
        try:
            y = int(self.year)
    ...
    def action_create_attendance(self):
        ...
        if not working_days:
            raise UserError(_(
                'There are no days found in the selected month.'
            ))
        ...
        # Compute month statistics: total calendar days, working days, Sundays, and public holidays
        try:
            y = int(self.year)
            m = int(self.month)
        except (ValueError, TypeError):
        ...
        if y and m:
            ...
            # Determine calendar and working weekdays for the employee
        ...
        message = _(
            ...
            'Calendar days: %(total)s, Working days: %(working)s, Public holidays: %(holidays)s, Sundays: %(sundays)s.'
        ) % {
            'count': created,
