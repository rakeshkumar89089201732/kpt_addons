...
class MonthlyAttendanceConfigWizard(models.TransientModel):
    ...
    export_scope = fields.Selection(
        ...
        default='all',
        required=True,
        help='Choose whether to export all accessible employees, only a specific department, or a single employee.',
    )
    department_id = fields.Many2one(
        'hr.department',
        string='Department',
        help='Department to filter employees when exporting the template.',
    )
    employee_id = fields.Many2one(
        'hr.employee',
        string='Employee',
        domain=lambda self: self._get_employee_domain(),
        help='Single employee to export when using the Single Employee scope.',
    )
    ...
    def action_export_template(self):
        ...
        # Apply export scope filters
        if self.export_scope == 'department':
            if not self.department_id:
                raise UserError(_("Please select a department to export."))
            domain.append(('department_id', '=', self.department_id.id))
        elif self.export_scope == 'employee':
            if not self.employee_id:
                raise UserError(_("Please select an employee to export."))
...
class MonthlyAttendanceEmployeeLine(models.TransientModel):
    ...
    def _get_working_days_in_month(self):
        """Return sorted list of all calendar days in the month.

        This is used to distribute Total Present Days across the month.
        Sundays and public holidays are also eligible days in this list.
        """
