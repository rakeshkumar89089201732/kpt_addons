...
def ks_read(self, *args, **kwargs):
    # Odoo calls Many2many.read(self, records). Be defensive in case of variations.
    records = args[0] if args else None
    if records is None:
        # Fallback to original behavior if somehow signature differs
    ...
    if self.name == 'ks_list_view_fields' or self.name == 'ks_list_view_group_fields':
        ...
        query = """ SELECT {rel}.{id1}, {rel}.{id2} FROM {rel}, {from_c}
                ...
                """.format(rel=self.relation, id1=self.column1, id2=self.column2,
                           ...
                           limit=(' LIMIT %d' % getattr(self, 'limit', None)) if getattr(self, 'limit', None) else '',
                           )
        ...
        # Build query using Odoo's Query API to keep parameters aligned
        ...
        # Join with the m2m relation table
        ...
        # Retrieve pairs and group by record
        group = defaultdict(list)
        records._cr.execute(query.select(sql_id1, sql_id2))
...
original_read_group = models.BaseModel._read_group_postprocess_groupby


def ks_read_group_postprocess_groupby(self, gb, query):
    """Properly override the _read_group_postprocess_groupby to ensure correct return format"""
    result = ks_time_addition(self, gb, query)
    # Make sure we return a model object, not a string
    if isinstance(result, dict) and 'groupby' in result and isinstance(result['groupby'], str):
        return original_read_group(self, gb, query)
    return result


# Replace the method with our custom implementation
models.BaseModel._read_group_postprocess_groupby = ks_read_group_postprocess_groupby
...
def ks_time_addition(self, gb, query):
    ...
    """
    split = gb.split(':')
    field_type = getattr(self, '_fields', {}).get(split[0], type(None))
    if field_type:
    ...
    # Use _field_to_sql instead of deprecated _inherits_join_calc
    field = self._fields.get(split[0])
    if field and field.store and field.type not in ('many2many', 'one2many'):
        qualified_field = self._field_to_sql(getattr(self, '_table', ''), split[0], query)
    else:
        qualified_field = '"' + getattr(self, '_table', '') + '"."' + split[0] + '"'
    if temporal:
        display_formats = {
            'minute': 'hh:mm dd MMM',
            'hour': 'hh:00 dd MMM',
