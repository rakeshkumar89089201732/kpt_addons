odoo.define('ks_dashboard_ninja.KsGlobalFunction', [], function () {
    "use strict";
    ...
    return {
        ...
        ks_monetary: function(value, currency_id) {
            // Fallback simple monetary formatting
            try {
        ...
        _onKsGlobalFormatter: function(ks_record_count, ks_data_format, ks_precision_digits){
            ...
            if (ks_data_format == 'exact'){
                return Number(ks_record_count).toFixed(ks_precision_digits);
            }else{
...
//                si[i].value = 1000000;
                ...
                if (negative) {
                    ...
                    if (num % 1===0){
                    num = Math.trunc(num).toString()
                    }else{
                        num = Number(num).toFixed(ks_precision_digits);
                    }
                    if (negative) {
                        return "-" + num;
                    } else {
