        ...
        for partner in partner_ids:
            ...
            for move_line in move_line_id:
                ...
                account_code = self.env['account.account'].browse(
                    move_line.account_id.id).name
                journal_code = self.env['account.journal'].browse(
                    move_line.journal_id.id).type.capitalize()
                if account_code:
                    move_line_data[0]['jrnl'] = journal_code
                    move_line_data[0]['code'] = account_code
                move_line_data[0]['id'] = partner.id
                move_line_data[0]['date'] = move_line_data[0]['date'] and move_line_data[0]['date'].strftime('%d-%b-%Y')
                move_line_data[0]['date_maturity'] = move_line_data[0]['date_maturity'] and move_line_data[0]['date_maturity'].strftime('%d-%b-%Y')
    ...
    def get_filter_values(self, partner_id, data_range, account, options):
        ...
        for partners in partner_id:
            ...
            if data_range:
                ...
                elif data_range == 'year' or data_range == 'groupMonth':
                    move_line_ids = self.env['account.move.line'].search(
                        [('partner_id', '=', partners), (
                            'account_type', 'in',
                            account_type_domain),
            ...
            balance = 0
            move_line_list = []
            if data_range == 'groupMonth':
                monthly_totals = defaultdict(lambda: {'date': '', 'move_name': '', 'debit': 0.0, 'credit': 0.0, 'account_type': '', 'date_maturity': '', 'account_id': '', 'journal_id': '', 'move_id': '', 'matching_number': '', 'amount_currency': ''})
                for line in move_line_ids:
                    month = line.date.strftime('%b-%Y')
                    monthly_totals[month]['debit'] += line.debit
                    monthly_totals[month]['credit'] += line.credit
                    monthly_totals[month]['date'] = month
                for month, values in monthly_totals.items():
                    move_line_list.append([values])
                # print(move_line_list)
            else:
