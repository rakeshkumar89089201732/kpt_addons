...
class PartnerLedger extends owl.Component {
        ...
        try {
            ...
            $.each(self.state.data, function (index, value) {
                if (index !== 'partner_totals') {
                    ...
                    ps_list.push({id: value[0][0].id, name: index})
                }
                else {
    ...
    this.state.searchText = ev.target.value.toLowerCase();
        ...
        debugger;
        if (!this.state.selectPartner.find(p => p.id === partner.id)) {
            this.state.selectPartner.push(partner);
        }
        this.state.partners = this.state.selectPartner.map(p => p.name);
        this.state.selected_partner = this.state.selectPartner.map(p => p.id);
        console.log(this.state.selectPartner)
        debugger;
        let selectedPartnerId = this.state.selected_partner;
        let partnerTotals = this.state.data.partner_totals;
        let selectedPartnerData = Object.values(partnerTotals).find((partner) => partner.partner_id === selectedPartnerId[0]);
        this.state.total_debit = selectedPartnerData ? selectedPartnerData.total_debit : 0;
        this.state.total_credit = selectedPartnerData ? selectedPartnerData.total_credit : 0;
//        let sp = this.state.selectPartner.map(p => p.name);
//        let totalDebitSum = 0;
//let totalCreditSum = 0;
...
removePartner = (partner) => {
    debugger;
    this.state.selectPartner = this.state.selectPartner.filter(p => p.id !== partner.id);
    this.state.searchText = ''
    this.state.partners = this.state.selectPartner && this.state.selectPartner.length
    ? this.state.selectPartner.map(p => p.name)
    : this.state.partner_list;
    const totalDebitSum = Object.values(this.state.data.partner_totals).reduce((sum, partner) => sum + (partner.total_debit || 0), 0);
    const totalCreditSum = Object.values( this.state.data.partner_totals).reduce((sum, partner) => sum + (partner.total_credit|| 0), 0);
    this.state.total_debit = totalDebitSum
    this.state.credit_debit = totalCreditSum
}


get dropdownOptions() {
    debugger;
    const selected = this.state.selectPartner || [];
    const search = this.state.searchText?.toLowerCase() || "";

    return this.state.ps.filter(p => {
        const name = String(p.name || "");
        return !selected.find(sp => sp.id === p.id) && name.toLowerCase().includes(search);
    });
}
    ...
    async applyFilter(val, ev, is_delete = false) {
        ...
        if (ev) {
            ...
            } else if (val.target.attributes["data-value"].value == 'groupMonth') {
