...
class AccountMove(models.Model):
    ...
    round_off_account_id = fields.Many2one(
        'account.account',
        string='Round Off Account',
        compute='_compute_kpt_rounding_compat',
        store=False,
    )
    ...
    def _compute_kpt_rounding_compat(self):
        for move in self:
            if move.invoice_cash_rounding_id:
                move.round_off_amount = move.invoice_cash_rounding_id.compute_difference(move.currency_id, move.amount_total or 0.0)
            else:
                move.round_off_amount = 0.0
            move.amount_total_rounded = (move.amount_total or 0.0) + move.round_off_amount

            rounding_account = False
            if move.invoice_cash_rounding_id:
                if move.round_off_amount >= 0:
                    rounding_account = move.invoice_cash_rounding_id.profit_account_id
                else:
                    rounding_account = move.invoice_cash_rounding_id.loss_account_id
            move.round_off_account_id = rounding_account
