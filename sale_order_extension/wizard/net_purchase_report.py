...
class NetPurchaseReport(models.TransientModel):
    _name = 'net.purchase.report'
    ...
    def generate_report(self):
        ...
        for month in months:
            ...
            vendor_debit_note_ids = self.env['account.move'].search([
            ...
            debit = sum(vendor_bill_ids.mapped('amount_total'))
            credit = sum(vendor_debit_note_ids.mapped('amount_total'))
        ...
        return self.env.ref('sale_order_extension.net_purchase_summary_pdf').report_action(self, data={'result': result, 'totals': totals, 'date_from': self.date_from, 'date_to': self.date_to}, config=False)
...
class NetPurchaseSummaryReport(models.AbstractModel):
    _name = 'report.sale_order_extension.report_net_purchase'
